...
#cetz.canvas(length: 1cm, {
  ...
  group({
    ...
    line((-1.5, 0), (9.5, 0), mark: (end: ">"), stroke: (thickness: 1pt))
    ...
    let height = 3.8
    let width = 25 // broad width in deg
    ...
    for (center_x, c) in pref_angles.zip(colors) {
      ...
      line(..pts, stroke: (paint: c, thickness: 1.5pt))
  ...
  // ------------------------------------------------------------------
  group({
    translate((12, 0)) // Shift right panel
    content((0, 6.2), text(weight: "bold", size: 10pt, "(B) Population Vector Sum"))
