...
#cetz.canvas(length: 1cm, {
  ...
  for i in range(n_lines) {
    ...
    if i == n_lines - 1 {
      content((-1.2, y - 1.7), text(weight: "bold",[Input\ Voltages]))
    }
  }
  ...
  for j in range(n_lines) {
    ...
    if j == 0 {
      content((x + 1.9, offset_y - 2.3), text(weight: "bold", [Output\ Currents]))
    }
  }
  ...
  // Callout line
  line((h_x + 0.43, h_y + 0.20), (h_x + 2.8, h_y + 1.2), stroke: (paint: gray ))
  ...
  group({
    ...
    content((1.0, 1.15), text([$I_i = V_j times G_(i j)$]), anchor: "west")

    // Accumulation
    content((0.2, 0.7), text([2. Accumulation (Kirchhoff's Law)]), anchor: "west")
    content((1.0, 0.15), text([$I_"total" = sum_i I_i$]), anchor: "west")
  })

  group({
    translate((6.6, 0.5))
    content((0.5, 1.5), text([Memory ($G$) and Compute ($I=V G$)\ are physically co-located.]), anchor: "west")
    content((0.5, 0.5), text(weight: "bold", [Zero data transport cost.]), anchor: "west")
  })
})
